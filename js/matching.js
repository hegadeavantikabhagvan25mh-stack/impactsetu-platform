...
// array (an easy mistake to make by hand in the console).
...
export function scoreMatch(ngo, csr, gaps = []) {
  ...
  const focusAreas = toArray(csr.focusAreas).map((f) => f.toLowerCase());
  ...
  const districts = toArray(csr.districts).map((d) => d.toLowerCase());
