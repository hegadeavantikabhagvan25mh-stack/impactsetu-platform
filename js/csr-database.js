...
// Tolerates a field saved as a plain string in Firestore instead of an
// array (an easy mistake to make by hand in the console) so a data-entry
// slip doesn't crash the whole table.
...
function populateCsrFilters() {
  ...
  const causes = [
    ...new Set(csrCompanies.flatMap((c) => toArray(c.focusAreas)).filter(Boolean)),
  ].sort();
...
export function renderCsrDatabase() {
  ...
  const filtered = csrCompanies.filter((c) => {
    ...
    const matchesCause = causeFilter === "All" || toArray(c.focusAreas).includes(causeFilter);
    const matchesQuery =
      !query ||
      (c.name || "").toLowerCase().includes(query) ||
      toArray(c.districts).some((d) => d.toLowerCase().includes(query));
  ...
  body.innerHTML = filtered
    .map(
      ...
      <tr>
        ...
        <td>${c.state || "—"}</td>
        <td>${toArray(c.focusAreas).join(", ") || "—"}</td>
        <td>${toArray(c.districts).join(", ") || "—"}</td>
      </tr>`
    )
    .join("");
}
